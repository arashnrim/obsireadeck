...
export class BookmarksService {
  ...
  async getReadeckData() {
    ...
    try {
      ...
      new Notice(
        `Readeck Importer: An error occured while getting bookmarks: ${error}`,
      );
      return;
    }
    ...
    if (bookmarksStatus.length <= 0) {
      if (this.settings.noticeVerbosity === "verbose")
        new Notice("Readeck Importer: No new bookmarks were found.");
      return;
    }

    // Ensure bookmarks folder exists
    const bookmarksFolder = this.app.vault.getAbstractFileByPath(
    ...
    if (get.md || get.annotations) {
      ...
      const bookmarksMPData = await this.getBookmarksData(
        toUpdateIds,
        get.md,
        get.res,
        true,
    ...
    for (const [id, bookmark] of bookmarksData.entries()) {
      ...
      if (bookmark.text || bookmark.annotations.length > 0) {
        ...
        const bookmarkFolderPath = this.settings.createBookmarkSubfolder
          ? `${this.settings.folder}/${id}`
          : this.settings.folder;
      ...
      if (bookmark.images.length > 0 && bookmark.json) {
        const bookmarkImgsFolderPath = this.settings.createBookmarkSubfolder
          ? `${this.settings.folder}/${id}/images`
          : `${this.settings.folder}/images`;
        await this.createFolderIfNotExists(
          bookmark.json.title,
          bookmarkImgsFolderPath,
        );
    ...
    if (this.settings.delete) {
      const toDeleteIds = bookmarksStatus
        .filter((b) => b.type === "delete")
        .map((b) => b.id);
  ...
  private async getBookmarkAnnotations(bookmarkId: string) {
    ...
    if (!annotations) {
      new Notice(
        `Readeck Importer: An unknown error occurred while getting annotations for ${bookmarkId}.`,
      );
    }
    return annotations;
  }
  ...
  private async getBookmarksData(
    ...
    const multipart = await this.api.getBookmarks(
      ids,
      markdown,
      resources,
      json,
  ...
  private async addBookmarkMD(
    ...
    await this.createFile(
      ...
      filePath,
      noteContent,
      this.settings.noticeVerbosity === "verbose",
    );
  }

  private async parseBookmarksMP(
    bookmarksData: Map<string, BookmarkData>,
    bookmarksMPData: any,
  ): Promise<boolean> {
    const partsData: MultipartPart[] =
      await Utils.parseMultipart(bookmarksMPData);
      ...
      annotationsContent =
        ...
        bookmarkAnnotations
          .map(
            (ann: any) =>
              `> ${ann.text}` +
              (this.settings.addLinkInAnnotations
