...
export class RDSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Ensure that there are no trailing slashes at the end of the URL.",
      )
      .addText((text) =>
        text
          ...
          .onClick(async () => {
            ...
            try {
              ...
              if (info.features?.includes("oauth")) {
                ...
                loginMode = "password";
                new Notice(
                  "Readeck Importer: OAuth is not supported on this Readeck instance. Consider upgrading your Readeck instance as password login will be removed soon.",
                );
              }
            } catch (err) {
            ...
            if (loginMode === "password") {
              try {
                new LoginModal(this.app, async (username, password) => {
                  const success = await this.plugin.auth.handleLogin(
                    username,
                    password,
                  );
                  if (success) {
            ...
            try {
              const authenticated = await this.plugin.auth.handleOAuth(
                client_name,
                (deviceAuth, onCancel) => {
                  const deviceCodeModal = new DeviceCodeModal(
                    this.app,
                    deviceAuth,
                    onCancel,
                  );
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        `If enabled, the bookmark will be stored in "${this.plugin.settings.folder}/<bookmark ID>/<bookmark name>.md". Otherwise, all bookmarks will be stored directly in "${this.plugin.settings.folder}/<bookmark name>.md".`,
      )
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.createBookmarkSubfolder)
          .onChange(async (value) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        `If enabled, the bookmark file names will be slugified to remove special characters and spaces. For example, "My Bookmark!" becomes "my-bookmark".`,
      )
      .addToggle((toggle) =>
        toggle
      ...
      .addButton((btn) => {
        ...
        btn
          ...
          .onClick(async () => {
            ...
            new Notice(
              "Readeck Importer: The last sync timestamp has been reset. The next sync will fetch all bookmarks.",
            );
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "If a bookmark was deleted in Readeck, delete the corresponding note in Obsidian.",
      )
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.delete).onChange(async (value) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Adds a link to the Readeck bookmark at the end of each annotation.",
      )
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.addLinkInAnnotations)
          .onChange(async (value) => {
...
class LoginModal extends Modal {
  constructor(
    ...
    new Setting(this.contentEl).setClass("b-0").addButton((btn) =>
      btn
...
class DeviceCodeModal extends Modal {
  ...
  constructor(
    ...
    instructions.createEl("p", {
      text: "1) Open the verification URL in your browser.",
    });
    const link = instructions.createEl("a", {
      text: this.device.verification_uri,
      href: this.device.verification_uri,
    });
    ...
    const actions = new Setting(this.contentEl)
      ...
      .addButton((btn) =>
        btn
          ...
          .onClick(() =>
            window.open(
              this.device.verification_uri_complete ||
                this.device.verification_uri,
              "_blank",
            ),
          ),
      )
