  ...
  async authorizeDevice(client_id: string): Promise<DeviceAuthorization> {
    ...
    switch (statusCode) {
      ...
      case 400:
        ...
        throw new Error(
          `OAuth device authorization failed: ${error.error_description}`,
        );
      default:
        throw new Error(
          `OAuth device authorization unexpected error: ${response.json}`,
        );
    }
  }
      ...
      switch (statusCode) {
        ...
        case 400:
          ...
          switch (body.error as oAuthErrorEnum) {
            ...
            case "access_denied":
              throw new Error(
                `OAuth device flow failed: ${body.error_description || body.error}`,
              );
            default:
              throw new Error(
                `OAuth device flow error: ${body.error_description || body.error}`,
              );
          }
          break;
        default:
